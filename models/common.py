...
class Bottleneck_YOLOX(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = DWConvblock(c_, c2, 3, 1)
        self.add = shortcut and c1 == c2 #从左到右计算表达式,若所有的都为真,则返回最后一个值,若存在假,返回第一个假值.
...
class C3_YOLOX(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(Bottleneck_YOLOX(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
